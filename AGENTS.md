...
7. Для скриншота Chrome использовать `GET /screenshot` или `POST /screenshot` у `@meta/chrome`, не напрямую в `@meta/screen`.
...
10. При ошибке `osascript failed (-1743)` — нет разрешения Automation.
11. При ошибке `osascript failed (-25211)` — нет разрешения Accessibility.
