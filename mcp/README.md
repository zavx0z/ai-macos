...
Текущий эксперимент — [fullscreen-прототип](viewer-prototype.md).
`zavx0z` выполняет команды без UI. `zavx0z_viewer` открывает приложение один раз;
приватный `zavx0z_viewer_next` доставляет новые ревизии без нового UI-шаблона.
Открытие fullscreen подтверждается ответом хоста после нажатия кнопки.
