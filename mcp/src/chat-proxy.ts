...
import { createChatExecutor, type ChatRuntimeOptions } from "./chat-executor.ts"
...
export async function startChatProxy(options: { catalogPath: string, runtime?: ChatRuntimeOptions }) {
  ...
  const executor = options.runtime ? createChatExecutor(snapshot, options.runtime) : undefined
    ...
    async callTool(name, args, signal) {
      ...
      try {
        ...
        if (request.action !== undefined) {
          if (!executor) throw new Error("Исполнитель не настроен")
...
if (import.meta.main) {
  ...
  const server = await startChatProxy({ catalogPath, runtime: {
