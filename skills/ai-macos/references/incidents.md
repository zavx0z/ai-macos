...
## 2026-09-15 — large image patches do not identify how evidence was generated

During a Dzen draft operation, the task history showed an approximately156k-character file-change patch containing the capture's base64 image, followed by a decoding command. The coordinator initially called this manual model transcription and a cause of delay. That inference was not supported: the history view did not expose the top-level code that produced the nested patch, and the operator reported programmatic storage. A large nested patch can be the correct result of `JSON.stringify`; its size alone does not prove resampling or explain latency.

The recommended method remains to preserve the original direct MCP response in a code variable: serialize it programmatically, save it through a file tool such as a programmatically constructed `tools.apply_patch`, and decode the image block from that saved response. Return the native image to the model for inspection and only short paths/status as text. Do not print the large JSON/base64 and then ask the model to reproduce it. Avoid passing the whole image in a shell command argument, which may exceed macOS ARG_MAX.
