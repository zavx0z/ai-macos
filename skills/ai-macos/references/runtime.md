...
`requestIssued` подтверждает вызов системного API, `missing` показывает права,
которые текущий helper ещё не подтвердил. Пользователь мог уже выдать доступ:
после обновления возможны смена идентичности подписи и состояние, сохранённое
старым процессом. Не утверждать, что пользователь не выдал права, только по
этому списку. Вызов request API не доказывает видимый диалог и не заменяет
положительные permission flags. `restart-needed`, `timed-out` и `failed`
не разрешают ввод; точную причину брать из health.

Новые bundle-релизы используют имя `computer-use.app`. Для диагностики брать
фактический helper path, build и cdhash из установленного runtime; не выбирать
строку системных настроек только по похожему имени. Исходники нового установщика
не доказывают, что bundle уже установлен. Готовность observer проверяется
отдельно от прав: все grants могут быть доступны при неготовом наблюдении.
