...
diff --git a/.ai/zavx0z.yaml b/.ai/zavx0z.yaml
new file mode 100644
index 0000000..a96651e
--- /dev/null
+++ b/.ai/zavx0z.yaml
@@ -0,0 +1,24 @@
+
...
+  }
+}
diff --git a/chrome/API.md b/chrome/API.md
new file mode 100644
index 0000000..d45283c
--- /dev/null
+++ b/chrome/API.md
@@ -0,0 +1,163 @@
+# @meta/chrome
...
+  }
+}
diff --git a/chrome/src/chrome.ts b/chrome/src/chrome.ts
new file mode 100644
index 0000000..320de28
--- /dev/null
...
+  };
+}
diff --git a/chrome/src/cli.ts b/chrome/src/cli.ts
new file mode 100644
index 0000000..ad07ad7
--- /dev/null
+++ b/chrome/src/cli.ts
@@ -0,0 +1,225 @@
+#!/usr/bin/env bun
...
+  process.exit(1);
+}
diff --git a/chrome/src/index.ts b/chrome/src/index.ts
new file mode 100644
index 0000000..b1a925d
--- /dev/null
+++ b/chrome/src/index.ts
@@ -0,0 +1,200 @@
+import {
+  activateTab,
+  closeTab,
...
+console.log(`  GET  /screenshot[?windowId=N&tabIndex=N&shadow=false&delayMs=200&format=png|json]`);
+console.log(`  POST /screenshot { windowId?, tabIndex?, shadow?, delayMs?, format?, restore? }`);
diff --git a/chrome/src/osascript.ts b/chrome/src/osascript.ts
new file mode 100644
index 0000000..4a8a401
--- /dev/null
+++ b/chrome/src/osascript.ts
@@ -0,0 +1,21 @@
+import { spawn } from "bun";
+
+export async function osa(script: string): Promise<string> {
...
+  }
+}
diff --git a/screen/API.md b/screen/API.md
new file mode 100644
index 0000000..75bb387
--- /dev/null
+++ b/screen/API.md
@@ -0,0 +1,269 @@
+# @meta/screen - REST API
...
+  }
+}
diff --git a/screen/src/capture.ts b/screen/src/capture.ts
new file mode 100644
index 0000000..c8841a8
--- /dev/null
+++ b/screen/src/capture.ts
@@ -0,0 +1,59 @@
+import { mkdir, mkdtemp, readFile, rm } from "node:fs/promises";
+import { tmpdir } from "node:os";
+import { join } from "node:path";
...
+  }
+}
diff --git a/screen/src/cli.ts b/screen/src/cli.ts
new file mode 100644
index 0000000..800a626
--- /dev/null
+++ b/screen/src/cli.ts
@@ -0,0 +1,104 @@
+#!/usr/bin/env bun
...
+  process.exit(1);
+}
diff --git a/screen/src/index.ts b/screen/src/index.ts
new file mode 100644
index 0000000..ef19189
--- /dev/null
+++ b/screen/src/index.ts
@@ -0,0 +1,228 @@
+import { captureDesktop, captureRect, type CaptureOptions } from "./capture.ts";
+import { frontmostApp } from "./restore.ts";
+import { createWindowApi, type WindowApi, type WindowInfo } from "./window-api.ts";
...
+  return new Promise((resolve) => setTimeout(resolve, ms));
+}
diff --git a/screen/src/restore.ts b/screen/src/restore.ts
new file mode 100644
index 0000000..c9b33eb
--- /dev/null
+++ b/screen/src/restore.ts
@@ -0,0 +1,23 @@
+import { spawn } from "bun";
+
+export async function frontmostApp(): Promise<string | null> {
...
+  return out.trim();
+}
diff --git a/screen/src/window-api.ts b/screen/src/window-api.ts
new file mode 100644
index 0000000..6b37422
--- /dev/null
+++ b/screen/src/window-api.ts
@@ -0,0 +1,67 @@
+export type WindowInfo = {
+  app: string;
+  pid: number;
...
+  return JSON.stringify(body);
+}
diff --git a/screen/tsconfig.json b/screen/tsconfig.json
new file mode 100644
index 0000000..9e25e6e
--- /dev/null
+++ b/screen/tsconfig.json
@@ -0,0 +1,4 @@
+{
+  "extends": "../../tsconfig.base.json",
+  "include": ["src/**/*.ts"]
...
+  }
+}
diff --git a/window/.claude/skills/macos-window/SKILL.md b/window/.claude/skills/macos-window/SKILL.md
new file mode 100644
index 0000000..c3f6aaf
--- /dev/null
+++ b/window/.claude/skills/macos-window/SKILL.md
@@ -0,0 +1,141 @@
+---
+name: macos-window
+description: Query, focus, move, resize, and arrange macOS windows by calling the @meta/window REST API (default http://localhost:7878). Use whenever the user asks to inspect or rearrange application windows on macOS.
...
+            └── osascript.ts   # thin spawn wrapper
+```
diff --git a/window/API.md b/window/API.md
new file mode 100644
index 0000000..4cde272
--- /dev/null
+++ b/window/API.md
@@ -0,0 +1,372 @@
+# @meta/window — REST API
...
+  http://localhost:7878/arrange
+```
diff --git "a/window/API\302\240\342\200\224 \320\272\320\276\320\277\320\270\321\217.txt" "b/window/API\302\240\342\200\224 \320\272\320\276\320\277\320\270\321\217.txt"
new file mode 100644
index 0000000..4cde272
--- /dev/null
+++ "b/window/API\302\240\342\200\224 \320\272\320\276\320\277\320\270\321\217.txt"	
@@ -0,0 +1,372 @@
+# @meta/window — REST API
+
+REST-сервис для управления окнами macOS. Структурирован под голосовое
...
+  http://localhost:7878/arrange
+```
diff --git a/window/package.json b/window/package.json
new file mode 100644
index 0000000..da6be56
--- /dev/null
+++ b/window/package.json
@@ -0,0 +1,17 @@
+{
+  "name": "@meta/window",
+  "private": true,
...
+  }
+}
diff --git a/window/src/cli.ts b/window/src/cli.ts
new file mode 100644
index 0000000..93bc3c0
--- /dev/null
+++ b/window/src/cli.ts
@@ -0,0 +1,178 @@
+#!/usr/bin/env bun
...
+  process.exit(1);
+}
diff --git a/window/src/index.ts b/window/src/index.ts
new file mode 100644
index 0000000..81c7015
--- /dev/null
+++ b/window/src/index.ts
@@ -0,0 +1,138 @@
+import { focusApp, getScreen, listWindows, moveWindow, raiseWindow, resizeWindow } from "./windows.ts";
+import { listPins, startPin, stopAllPins, stopPin } from "./pin.ts";
+
...
+console.log(`  GET  /pin                                  list active pins`);
+console.log(`  DEL  /pin/:id  | DEL /pin                  stop one / all pins`);
diff --git a/window/src/osascript.ts b/window/src/osascript.ts
new file mode 100644
index 0000000..d7ea6cc
--- /dev/null
+++ b/window/src/osascript.ts
@@ -0,0 +1,17 @@
+import { spawn } from "bun";
+
+export async function osa(script: string): Promise<string> {
...
+  return out.trim();
+}
diff --git a/window/src/pin.ts b/window/src/pin.ts
new file mode 100644
index 0000000..474aa62
--- /dev/null
+++ b/window/src/pin.ts
@@ -0,0 +1,65 @@
+import { raiseWindow } from "./windows.ts";
...
+  return rest;
+}
diff --git a/window/src/windows.ts b/window/src/windows.ts
new file mode 100644
index 0000000..e444226
--- /dev/null
+++ b/window/src/windows.ts
@@ -0,0 +1,109 @@
+import { osa } from "./osascript.ts";
+
+export type WindowInfo = {
...
+  return { width: parts[2] ?? 0, height: parts[3] ?? 0 };
+}
diff --git a/window/tsconfig.json b/window/tsconfig.json
new file mode 100644
index 0000000..9e25e6e
--- /dev/null
+++ b/window/tsconfig.json
@@ -0,0 +1,4 @@
+{
+  "extends": "../../tsconfig.base.json",
+  "include": ["src/**/*.ts"]
