...
test("Runtime host serves schema catalogue, method health and per-lineage frames", async () => {
  ...
  try {
    ...
    expect((await client.listTools()).map(tool => tool.name)).toEqual(["get_state", "observe", "get_target_status", "cancel_target", "recover_startup_input", "system_health", "list_recent_operations", "get_operation", "cancel_operation", "agent_request"])
