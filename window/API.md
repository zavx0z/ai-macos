...
## Устойчивый идентификатор окна

Основной selector: `{app, pid, windowId}` из свежего `GET /windows`.
`windowId` — системный `CGWindowID` (положительный UInt32), сохраняющийся у
живого окна при смене заголовка, перемещении, размере и перестановке AXWindows.
`index` остаётся совместимым диагностическим номером, но не identity.

Все операции `/focus`, `/raise`, `/move`, `/resize`, `/arrange`, `/pin` принимают
`windowId`. При наличии ID изменяемые подсказки index/title/frame игнорируются;
app и pid проверяются. Отсутствующий ID не разрешает fallback на другое окно.
Неоднозначное соответствие CG/AX отклоняется. Значение 0 в наблюдении означает
«ID недоступен» и недопустимо в запросе. ID не переносится между пользовательскими
сессиями macOS и не переиспользуется после закрытия окна.

`GET /frontmost` возвращает также `ownerWindowId` для вложенного native sheet.
Связь определяется по AX parent/sheets, а не только совпадению прямоугольников.
Для ввода в Save/Open используется selector родительского окна из `/windows`.

Внутренний input API принимает `{pid, windowId}` вместо legacy `{pid,index}`.
Native helper принимает `window-focus <pid> id:<windowId>` (аналогично raise,
move, resize). Это внутренний интерфейс разработки, не обход direct MCP.
...
`index: <n>` из текущей инвентаризации. Он не является устойчивым ID и не гарантирует порядок по фокусу.

---

## 2. Нормализация имён приложений

Имя `app` берётся из нативной инвентаризации процессов (как видно в
...
Сравнение app регистронезависимо. Возвращённые канонические app/pid/windowId
передаются дальше без угадывания процесса.
